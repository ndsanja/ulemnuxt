...
<template>
  <BuilderToolsStylesPanelEditor v-if="toBottom" class="fixed z-40 left-[5px] bottom-[5px] rounded w-[250px]"
    ...
    <template #title>
      <div class="flex items-center w-full">
        ...
        <div v-if="toBottom" @click="clickExpanded">expanded</div>
      </div>
    </template>
    ...
    <BuilderToolsStylesPanelEditor>
      <template #title>
        <div class="flex items-center w-full" :ondrag="positionX = x">
          <p :ondrag="positionY = y" ref="handle" :class="{ 'cursor-move': true }" class="flex-1">Element
            {{
              dataById.id
            }}
          </p>
          <div v-if="!toBottom" @click="clickToBottom">bottom</div>
          <div @click="toogleEditor = !toogleEditor" class="ml-2 cursor-pointer">hide</div>
        </div>
      </template>
        ...
        <div class="flex items-center justify-between w-full">
          ...
          <div v-if="!isFit" un-cloak class="text-xs flex items-center justify-end space-x-2">
            <div v-if="dataById.classes.xs.length" @click="setDisplay('xs')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': display == 'xs' }">xs</div>
            <div v-if="dataById.classes.sm.length" @click="setDisplay('sm')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': display == 'sm' }">sm</div>
            <div v-if="dataById.classes.md.length" @click="setDisplay('md')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': display == 'md' }">md</div>
            <div v-if="dataById.classes.lg.length" @click="setDisplay('lg')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': display == 'lg' }">lg</div>
            <div v-if="dataById.classes.xl.length" @click="setDisplay('xl')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': display == 'xl' }">xl</div>
          </div>
          <div v-if="isFit" un-cloak class="text-xs flex items-center justify-end space-x-2">
            <div v-if="dataById.classes.xs.length" @click="setDisplay('xs')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': displaySize < Breakpoints.sm }">xs</div>
            <div v-if="dataById.classes.sm.length" @click="setDisplay('sm')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': displaySize >= Breakpoints.sm && displaySize < Breakpoints.md }">
              sm</div>
            <div v-if="dataById.classes.md.length" @click="setDisplay('md')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': displaySize >= Breakpoints.md && displaySize < Breakpoints.lg }">
              md</div>
            <div v-if="dataById.classes.lg.length" @click="setDisplay('lg')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': displaySize >= Breakpoints.lg && displaySize < Breakpoints.xl }">
              lg</div>
            <div v-if="dataById.classes.xl.length" @click="setDisplay('xl')"
              :class="{ 'cursor-pointer': true, 'text-sm font-bold': displaySize >= Breakpoints.xl }">xl</div>
          </div>
        </div>
      </template>

      <div :class="{ 'bg-slate-700 min-h-screen': true, '!min-h-full': true }">
