...
<template>
  <Element v-for="(item, index ) in props.data" :key="item.id"
    ...
    @click.self="id = item.id" :data-item="item.id" :data-index="index" :data-parent="item.parentId" :elements-data="item"
    :draggable="`${item.isRoot ? false : true}`" @dragstart.self="useOnDragStart($event, false)"
    ...
    @dragleave.self.prevent="useOnDragLeave($event)" @dragend.self.prevent="useOnDragEnd($event)" class="draggable">
