...
const { id, hoverId, activeId, handleDelete, dataClasses, textWidth, textContent, fontContent, getDataById, getTextWidth } = useStore()
...
<template>
  <ClientOnly>
    <ElementEditable v-for="(item, index) in props.data" :key="item.id" :dataItemProps="item" :data-itemId="item.id"
      ...
      :style="item.elKind == 'p' || item.elKind == 'h1' || item.elKind == 'h2' || item.elKind == 'h3' || item.elKind == 'h4' || item.elKind == 'h5' || item.elKind == 'h6' ? `width: calc(${getTextWidth(item.content, fontContent)}px + 0.75ch); max-width: 100% ; min-width: 1ch` : ''">
